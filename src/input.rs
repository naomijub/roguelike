...
pub fn check_player_input(
    ...
    if input.any_just_pressed([KeyCode::Right, KeyCode::D]) && can_move_right(&player_position, map)
    {
        move_right(&mut player_position);
        next_state.set(GameState::EnemyTurn);
    }

    if input.any_just_pressed([KeyCode::Left, KeyCode::A]) && can_move_left(&player_position, map) {
        move_left(&mut player_position);
        next_state.set(GameState::EnemyTurn);
    }

    if input.any_just_pressed([KeyCode::Up, KeyCode::W]) && can_move_up(&player_position, map) {
        move_up(&mut player_position);
        next_state.set(GameState::EnemyTurn);
    }

    if input.any_just_pressed([KeyCode::Down, KeyCode::S]) && can_move_down(&player_position, map) {
